...
define('BCS_BUCKET','qukutest');//测试bucket,线上bucket暂时保密
define('BCS_HOST','10.81.2.114:8090');//测试地址,线上暂时保密

//$host = 'bcs.duapp.com'; //online
$host = '10.81.2.114:8090'; //test
...
//	$response = $baidu_bcs->create_bucket ( $bucket, $acl, $opt );
...
function delete_bucket($baidu_bcs) {
	global $bucket;
	$opt = array ();
	$response = $baidu_bcs->delete_bucket ( $bucket, $opt );
	printResponse ( $response );
}
function list_object($baidu_bcs) {
	global $bucket, $fileWriteTo;
	$opt = array (
			'start' => 0, 'limit' => 5, 'prefix' => '/a' );
...
function create_object($baidu_bcs) {
	global $fileUpload, $object, $bucket;
				$file_name='qukutest.txt';
	$opt = array (
//			'acl' => BaiduBCS::BCS_SDK_ACL_TYPE_PUBLIC_WRITE, 
			BaiduBCS::IMPORT_BCS_LOG_METHOD => "bs_log" ,
...
function upload_directory($baidu_bcs) {
	...
	$opt = array (
			"prefix" => "/20110622", "has_sub_directory" => true, 
...
function copy_object($baidu_bcs) {
	...
	$dest = array (
			'bucket' => $bucket, 'object' => $object . 'copy' );
...
function set_object_meta($baidu_bcs) {
	...
	$meta = array (
			"Content-Type" => MimeTypes::get_mimetype ( "pdf" ) );
...
function generate_get_object_url($baidu_bcs) {
	global $bucket, $object;
	$opt = array (
			"time" => time () + 3600,  //链接生效时间为linux时间戳向后一小时 
			//"ip" => "192.168.1.1"    //限制本链接发起的客户端ip
	);
...
function generate_put_object_url($baidu_bcs) {
	...
	$opt = array (
			//"time" => time () + 3600,//链接生效时间为linux时间戳向后一小时 
			//"size" => 1024 * 1024,   //主要用户上传时，限制上传大小，这里限制1MB
			//"ip" => "192.168.1.1"    //限制本链接发起的客户端ip
	);
